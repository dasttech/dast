...
contract AssetStore {
    ...
    address admin;
    // users
    mapping(address => StorageStruct.User) users;
    uint256 userCount;
    //ownership by email=>address
    mapping(string => address) ownerByEmail;
    // msg.sender=>Asset
    mapping(address => StorageStruct.Asset[]) assets;
    // asset's token =>msg.sender
    mapping(string => address) public owners;
    // asset's token => trusted contaccts
    mapping(string => StorageStruct.Contact[]) contacts;
    ...
    public returns(bool){
        ...
        users[tx.origin] = StorageStruct.User(userCount++,tx.origin,email,phone,bio);
    ...
    function store(
        ...
        string memory title,
        string memory asset,
        StorageStruct.Contact[] memory trusted_contacts
    ) public returns (bool) {
    ...
    function fetch()
        public
        view
        returns (StorageStruct.Asset[] memory, StorageStruct.Contact[][] memory)
    {
        StorageStruct.Asset[] memory asset = assets[tx.origin];
        StorageStruct.Contact[][]
            memory contact = new StorageStruct.Contact[][](asset.length);
    ...
    function edit(
        ...
        assets[owner][id] = StorageStruct.Asset({
        ...
        for (uint256 i = 0; i < counter; i++) {
               ...
               contacts[token].push(StorageStruct.Contact(
                    i,
                    trusted_contacts[i].email,
                    trusted_contacts[i].phone
                ));
